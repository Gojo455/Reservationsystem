...
def _lock_key(showtime_id: int, row: str, col: int) -> tuple:
...
def get_seat_lock_status(showtime_id: int, row: str, col: int,
...
def compute_recommendation_scores(user_id: int,
                                   request_date: date,
                                   selected_genres: list[str] = None) -> list[dict]:
    """
    ...
    my_booked_movie_ids: set[int] = set()
...
def update_user_preferences(user_id: int, movie_genres: str):
    """
...
def _paystack_request(endpoint: str, payload: dict = None, method: str = 'POST') -> dict:
